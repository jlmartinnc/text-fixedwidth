...
is join("|",$fw->attributes), 'points',         'attributes()';
is $fw->max_len,        1,                      'max_len()';
...
is join("|",$fw->attributes), 'points|points2', 'attributes()';
is $fw->max_len,        8,                      'max_len()';


# Now let's try a combo reassign:
ok $fw->set_attribute(
   name    => 'points2',
   default => 'undef',
   # Use %07.0f not %07d or you may suffer off-by-a-penny rounding errors.
   # Thanks mauke in irc.perl.org #perl-help
   reader  => sub { sprintf("%07.0f", ($_[0]->get_points2 + 1) * 100) },
   length  => 7,
),                                              'set_attribute() points2 w/ reader';
is $fw->get_points2,    undef,                  'get_points2()';
ok $fw->set_points2(69.99),                     'set_points2()';
is $fw->get_points2,    69.99,                  'get_points2()';
is $fw->getf_points2,   '0007099',              'getf_points2()';
is $fw->string,         '60007099',             'string()';

ok $fw->set(points2 => 71.99),                  'set(points2 => 71.99)';
is $fw->get("points2"), 71.99,                  'get("points2")';
is $fw->getf("points2"),'0007299',              'getf("points2")';
is $fw->string,         '60007299',             'string()';
